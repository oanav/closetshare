...
$(document).ready(function(){
    $(".sub_button").click(function(){
        $.post('<?php echo osc_base_url(true); ?>', {email:$("#alert_email").val(), userid:$("#alert_userId").val(), alert:$("#alert").val(), page:"ajax", action:"alerts"},
            function(data){
                if(data==1) { swal('<?php echo osc_esc_js(__('You have sucessfully subscribed to the alert', 'osclasswizards')); ?>'); }
                else if(data==-1) { swal('<?php echo osc_esc_js(__('Invalid email address', 'osclasswizards')); ?>'); }
                else { swal('<?php echo osc_esc_js(__('There was a problem with the alert', 'osclasswizards')); ?>');
                };
        });
        return false;
    });
