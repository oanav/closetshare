...
// Before HTML
   ...
   function osclass_pm_before_html() {
      ...
      if(osc_is_web_user_logged_in() && pmFlashAlert() == 1 && !osclass_pm_is_inbox() && !osclass_pm_is_messages() ) {
         ...
         if($countPMs > 0 ) {
			 osc_add_flash_ok_message('<a href="'.osc_render_file_url(osc_plugin_folder(__FILE__) . 'user-inbox.php').'" >'.sprintf(_n('Imate %d novu poruku!', 'Imate %d nove poruke!', $countPMs), $countPMs).'</a>');
         }
      }
   }
...
// Everything between this section is user side related.
    ...
    function osclass_pm_header() {
       ...
       // we only load the javascript when we need it.
       		if(osclass_pm_is_drafts() || osclass_pm_is_send() || osclass_pm_is_messages() || osclass_pm_is_pmSettings() ) {
       ?>
    ...
    */ ?>
    ...
    function osclass_pm_new_user($userID) {
		ModelPM::newInstance()->insertUserPmSettings($userID, 1, 0, 0);
	}
...
// End admin side related.
    ...
    function osclass_pm_cron() {
	   // deletes messages that have been deleted by both the sender and reciver.
	...
	osc_add_hook('user_register_completed', 'osclass_pm_new_user');
    osc_add_hook('admin_header', 'osclass_pm_admin_header');
    // Add hook to supertoolbar
    osc_add_hook('supertoolbar_hook' , 'osclass_pm_supertoolbar');

    //3.0 menu structure
    if(osc_version() >= 300) {
		osc_add_hook('admin_menu_init', 'init_menu_pm');
	}

	function init_menu_pm() {
		osc_add_admin_menu_page(
		   pmAdmin() . __('\'s PM Box','osclass_pm'),
		   osc_admin_render_plugin_url('osclass_pm/admin-inbox.php'),
		   'osclass_pm',
		   null,
		   null,
		   1
		...
		if(pmDrafts()){
			osc_add_admin_submenu_page(
			   'osclass_pm',
			   __('Drafts', 'osclass_pm'),
			   osc_admin_render_plugin_url('osclass_pm/admin-drafts.php'),
			   'osclass_pm-drafts'
			) ;
		}
		...
		osc_add_admin_submenu_page(
		   'osclass_pm',
		   __('Outbox', 'osclass_pm'),
		   osc_admin_render_plugin_url('osclass_pm/admin-outbox.php'),
		   'osclass_pm-outbox'
		) ;
		...
		osc_admin_menu_settings(
			__('PM Settings', 'osclass_pm'),
			'#',
			'osclass_pm-settingsHead'
		);

		osc_admin_menu_settings(
			__('Configure', 'osclass_pm'),
			osc_admin_render_plugin_url('osclass_pm/admin.php'),
			'osclass_pm-config'
		);

		osc_admin_menu_settings(
			__('Help', 'osclass_pm'),
			osc_admin_render_plugin_url('osclass_pm/help.php'),
			'osclass_pm-help'
		);
	}
