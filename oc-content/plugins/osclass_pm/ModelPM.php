<?php
    ...
    class ModelPM extends DAO
    {
         ...
         * @return array 
         */
        public function getByThreadId($thread_id, $del=NULL, $orderBy = null, $oDir = null)
        {
            $this->dao->select();
            $this->dao->from( $this->getTable_pmMessages());
            $this->dao->where('pm_thread', $thread_id);
            if($del == 1) {
               $this->dao->where('senderDelete', 0);
               $this->dao->where('recipDelete', 0);
            }
         ...
         */
        public function insertMessage($thread_id, $sender_id, $recip_id, $pm_subject, $pm_message, $senderDelete = 1)
        {
            $this->dao->insert($this->getTable_pmMessages(), array('thread_id' => $thread_id,'sender_id' => $sender_id, 'recip_id' => $recip_id, 'pm_subject' => $pm_subject, 'pm_message' => $pm_message, 'senderDelete' => $senderDelete)) ;
        }
         ...
         * @param int $user_id, $emailAlert, $flashAlert, $saveSent 
         */
        public function insertUserPmSettings($user_id, $emailAlert, $flashAlert, $saveSent)
        {
            $this->dao->insert( $this->getTable_pmSettings(), array('fk_i_user_id' => $user_id, 'send_email' => $emailAlert, 'flash_alert' => $flashAlert, 'save_sent' => $saveSent));
        }        
